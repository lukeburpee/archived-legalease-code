...
class TreeListImpl extends Component {
    constructor(props) {
        super(props)
    ...
    handleContextMenu(event, listItem){
    	event.preventDefault();
        this.props.dispatch(actions.openFileTreeContext(event.currentTarget, listItem));
    }
    render() {
        // required props
                ...
                if (listItem._shouldRender) {
                    return (
                        ...
                        <ListItem
                            ...
                            leftIcon={getLeftIcon(listItem, this.props.useFolderIcons, this.props.useSearchIcons, listItem.isPrivate, listItem.isLocked)}
                            ...
                            onContextMenu={(event)=> {this.handleContextMenu(event, listItem)}} />
                            <FileTreeContext listItem={listItem}/>
                            </div>                   
                    )                   
                } else {
                    return null
        ...
        function getLeftIcon(listItem, useFolderIcons, useSearchIcons, isPrivate, isLocked) {
            if (useFolderIcons) {
                if (listItem.children) {
                	if (useSearchIcons){
                        if (isLocked){
                            return <object type="image/svg+xml" data="/icons/ic_search_folder_locked.svg"/>
                        } else {
                    	   return <object type="image/svg+xml" data="/icons/ic_search_folder.svg" />
                        }
                    } else {
                        if (isLocked){
                            return <object type="image/svg+xml" data="/icons/ic_folder_locked.svg"/>
                        } else {
                    	   return <object type="image/svg+xml" data="/icons/folder.svg" />
                        }
                    }
                } else if (useSearchIcons){
                	return <object type="image/svg+xml" data="/icons/ic_search.svg" />
                } else {
                    return <FileIcon />
                }
            } else {
...
TreeListImpl.contextTypes = {
    muiTheme: PropTypes.object
}

TreeListImpl.propTypes = {
    ...
    useSearchIcons: PropTypes.bool,
    haveSearchbar: PropTypes.bool,
    searchTerm: PropTypes.string
}

const TreeList = connect()(TreeListImpl);
