...
import NavigateFirst from 'material-ui/svg-icons/navigation/first-page';
import NavigateLeft from 'material-ui/svg-icons/image/navigate-before';
import NavigateLast from 'material-ui/svg-icons/navigation/last-page';
...
class MuiDataTableImpl extends React.Component {
  constructor(props) {
    super();
  ...
  componentWillReceiveProps(nextProps){
    let tableData = nextProps.config.data || [];
    let rowsPerPage = nextProps.config.paginated.constructor === Object ? nextProps.config.paginated.rowsPerPage : 5;

    tableData = nextProps.config.paginated ? new Paginate(tableData).perPage(rowsPerPage) : tableData;

    if (tableData instanceof Paginate) {
      tableData = tableData.page(1);
    }
    this.setState({
      idempotentData: nextProps.config.data,
      paginatedIdempotentData: new Paginate(nextProps.config.data),
      perPageSelection: nextProps.config.paginated.rowsPerPage || 5,
      tableData: tableData,
      searchData: []
    });
  }
  ...
  componentDidMount(){
    const tableBody = ReactDOM.findDOMNode(this.refs.body).parentNode.parentNode;
  ...
  componentWillUnmount(){
    this.viewport.removeEventListener('scroll',(e)=> {
      this.handleScroll()
    })
  }
  ...
  handleScroll(){
    ...
    dispatch(actions.setDataTableScrolling());
    setTimeout(function(){dispatch(actions.setDataTableScrollingFinished())}, 5000);
  }
      ...
      <Paper zDepth={1}>
        <Table
          ...
          <TableFooter style={this.shouldShowItem(this.props.config.paginated)}>
            <TableRow>
              <TableRowColumn
              	style={{textAlign: 'left', verticalAlign:'middle', width: '10%'}}>
              	<SelectField
              		style={{width:100}}
              		value={this.state.docsToEdit}>
              		<MenuItem value={0} primaryText="Selected"/>
              		<MenuItem value={1} primaryText="Current Page"/>
              ...
              <TableRowColumn
              	...
              	<SelectField
              		style={{width:60}}
              		value={this.state.docChangeOptionSelected}>
              		<MenuItem value={0} style={{overflowX:'hidden'}} primaryText="Edit"/>
              		<MenuItem value={1} style={{overflowX:'hidden'}} primaryText="Stats"/>
              		<MenuItem value={2} style={{overflowX:'hidden'}} primaryText="To Excel"/>
              		<MenuItem value={3} style={{overflowX:'hidden'}} primaryText="To PDF"/>
              	</SelectField>
              </TableRowColumn>
              <TableRowColumn
              	style={{textAlign: 'left', verticalAlign:'middle', width: '10%'}}>
              ...
              <TableRowColumn
                style={{ textAlign: 'right', verticalAlign: 'middle', width: '40%' }}
              >
              ...
              <TableRowColumn style={{ textAlign: 'right', verticalAlign: 'middle' }}>
              	<NavigateFirst style={this.state.navigationStyle}/>
                ...
                <NavigateLast style={this.state.navigationStyle}/>
              </TableRowColumn>
            </TableRow>
          </TableFooter>
...
const MuiDataTable = connect(mapStateToProps)(MuiDataTableImpl);
