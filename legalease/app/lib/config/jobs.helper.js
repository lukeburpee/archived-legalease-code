...
export const createJobs = (file) => {
  ...
  if (file.meta.convertPst){
    createPstConvertJob(file);
  }
  if (file.meta.convertDwg){
    createDwgConvertJob(file);
  }
}
...
const createPstConvertJob = (file) => {
  const extractPstJob = new Job(DiscoveryJobs, 'convertPst',{
    _id:file._id,
    name:file.name,
    type:file.type,
    database: file.meta.database
  ...
  extractPstJob.retry({retries:50, wait:300*1000})
  extractPstJob.save();
}

const createDwgConvertJob = (file) => {
  const extractPstJob = new Job(DiscoveryJobs, 'convertDwg',{
    _id:file._id,
    name:file.name,
    type:file.type,
    database: file.meta.database
  ...
  extractPstJob.priority('normal')
  extractPstJob.retry({retries:50, wait:250*1000})
  extractPstJob.save();
}