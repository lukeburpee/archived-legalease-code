...
export default function () {
	Meteor.methods({
		...
		'annotate.text': function(fileId, text){
			...
			let annotations = documentAnalyzeAPI(text);
			console.log(annotations);
		},
		'extract.text': function(id, name, type, annotateText){
			...
			if (annotateText){
				...
				annotationJob.retry({retries:50, wait:60000});
				annotationJob.save();
			}
			return text;
		},
		...
		'convert.pst': function(id, name, type){
			const fileId = id;
			console.log('starting pst conversion for: ' + fileId);
			let readStream = getFileReadStream(fileId);
			var req = request.post(Meteor.settings.PST_SERVER);
			var form = req.form();
			form.append('filename', name);
			form.append('mimetype', type);
			form.append('pst_file', readStream);
		},
		...
		'convert.dwg': function(id, name, type){
			const fileId = id;
			console.log('starting dwg conversion for: ' + fileId);
			let readStream = getFileReadStream(fileId);
			let fileOutputName = name.slice(0, name.indexOf('.')) + '.pdf';
			console.log(fileOutputName);
			const metadata = { versionName: 'pdf', discoveryfileId: id, uploadDate: new Date() };
			...
			let stream = convertDwgAPI(type, 'application/pdf');
			stream.on('error', function(error){console.log(error); });
			readStream.pipe(stream);
			stream.pipe(writeStream);
			writeStream.on('error', function(error){console.log(error); });
...
function getFileReadStream(id){
	...
	if (gridId){
		if (file.meta.database === 'default'){
			...
			console.log('search external gridfs for: ', String(gridId));
			const url = 'mongodb://mongo1:27017,mongo2:27017,mongo3:27017/' +  file.meta.matter + '?replicaSet=appRS';
			const database = new MongoInternals.RemoteCollectionDriver(url).mongo.db;
			const externalGfs = Grid(database, MongoInternals.NpmModule);
			const readStream = externalGfs.createReadStream({gridId});
			readStream.on('error', err => { console.log(err); });
...
function convertDwgAPI (inputType, outputType, data, callback){
	var	options = {
			url: Meteor.settings.DWG_SERVER,
			headers: {
				'Content-Type': inputType,
				'Accept': outputType
			}
...
function pstConvertAPI (data, callback){
	var	options = {
		url: Meteor.settings.PST_SERVER,
	};
	var req = request.post(options);
	var form = req.form();
	form.append('filename', 'pst.zip');
	return request.post(options);
};
