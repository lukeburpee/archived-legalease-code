...
function doDWGJob(job) {
  console.log('DWG converter process a job from the queue', {
    outputExtension: job.outputExtension,
    tempPath: job.tempPath
  });

  active = true;
  var child = spawn('./dwg2pdf.sh', [
    '--stdout',
    '-auto-fit',
    '-paper=Letter'
  ]);
  ...
  child.on('error', function(err) {
    console.warn('DWG spawn child failed', err);
    finishJob(job, err);
  });

  child.on('exit', function(code) {
    console.log('DWG spawn child exited with code', code);
    finishJob(job);
  });
  ...
  child.stderr.on('data', function(data) {
    console.warn('DWG converter received message on stderr', data.toString);
  });
}
