...
ddp.connect(function(err, wasReconnect){
  ...
  var dwgWorkers = Job.processJobs('discoveryJobsQueue', 
    'convertDwg', 
    { 
      callbackStrict: true,
      pollingInterval: 6000,
...
var convertPstWorker = function(job, cb){
  console.log('convert pst file: ' + job.data._id);
  job.progress(30, 100, {echo:true}, function(err, result){if(err){console.log(err)}});
  ddp.call('convert.pst', 
  [job.data._id, job.data.name, job.data.type], 
  function(err, result){
    if (err){
      console.log('convert pst job failed with error: ' + err);
      job.fail('convert pst job failed with error: ' + err);
      cb();
    }
    console.log('called convert pst function, result: ' + result);
  job.progress(90, 100, {echo:true}, function(err, result){if(err){console.log(err)}});
  },
  function(){
      job.progress(60, 100, {echo:true}, function(err, result){if(err){console.log(err)}});
    });
  console.log('finished convert pst job: ' + job.data._id);
  job.done('finished convert pst method for file: ' + job.data._id);
  cb();
}

var convertDwgWorker = function(job, cb){
  console.log('converting dwg file: ' + job.data._id);
  job.progress(30, 100, {echo:true}, function(err, result){if(err){console.log(err)}});
  ddp.call('convert.dwg', 
  [job.data._id, job.data.name, job.data.type], 
  function(err, result){
    if (err){
      console.log('convert dwg job failed with error: ' + err);
      job.fail('convert dwg job failed with error: ' + err);
      cb();
    }
    console.log('called convert dwg function, result: ' + result);
  job.progress(90, 100, {echo:true}, function(err, result){if(err){console.log(err)}});
  },
  function(){
      job.progress(60, 100, {echo:true}, function(err, result){if(err){console.log(err)}});
    });
  console.log('finished convert dwg job: ' + job.data._id);
  job.done('finished convert dwg method for file: ' + job.data._id);
  cb();
}
