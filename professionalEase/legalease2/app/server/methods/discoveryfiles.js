...
import corenlp from 'corenlp-js-interface';
...
export default function () {
	Meteor.methods({
		...
		'translate': Meteor.wrapAsync(function(inputFile, language){
 		...
 		'extract.pst': function(id, name, type){
			const fileId = id;
			console.log('starting pst extraction for: ' + fileId);
			let readStream = getFileReadStream(fileId);
			let stream = documentExtractPstAPI(type, 'application/pdf');
			stream.on('error', function(error){console.log(error); });
			readStream.pipe(stream);
		},
...
function documentExtractPstAPI (inputType, outputType, data, callback){
	var	options = {
...
function documentAnalyzeAPI(text){
	let results = corenlp(text, 9000/*port*/,"parse,natlog,ssplit"/*annotators*/,"json"/*format*/);
