(function ()
{
    ...
    /** @ngInject */
    function DashboardProjectController($scope, $interval, $mdSidenav, DashboardData, $mdMedia, $mdDialog, Auth)
    {
        $scope.status = '  ';
        ...
        vm.projects = vm.currentUser.clients;
        ...
        $scope.showUpdateDialog = function(ev) {
            ...
            templateUrl: 'app/main/apps/dashboards/project/update.html',
    ...
    function updateController($scope, Auth, Restangular, $mdDialog) {
        ...
        $scope.createClientForm = {
            id : String($scope.currentUser._id)
        };
        $scope.createReviewForm = {};
        $scope.createUserForm = {};
        if ($scope.role === 'primary') {
            $scope.allowedRoles = ['primary', 'clientManager','partner', 'associate', 'reviewManager', 'reviewer'];
        };
        if ($scope.role === 'clientManager') {
            $scope.allowedRoles = ['clientManager', 'partner', 'associate', 'reviewManager','reviewer'];
        };
        var baseClient = Restangular.all('clients');
        var baseReview = Restangular.all('reviews');
        var baseUser = Restangular.all('users');
        $scope.createClient = function() {
            baseClient.post($scope.createClientForm);
        };
        $scope.createReview = function() {
            baseReview.post($scope.createReviewForm);
        };
        $scope.createUser = function() {
            baseUser.post($scope.createUserForm);
        };
    }
})();