...
if [ $? -eq 1 ]; then
    gosu $NUXEO_USER $NUXEO_HOME/bin/nuxeoctl mp-install /docker-entrypoint-initnuxeo.d/nuxeo-api-playground-*.zip --relax=false --accept=true
fi
...
gosu $NUXEO_USER $NUXEO_HOME/bin/nuxeoctl mp-list|grep showcase-content
if [ $? -eq 1 ]; then
    gosu $NUXEO_USER $NUXEO_HOME/bin/nuxeoctl mp-install /docker-entrypoint-initnuxeo.d/nuxeo-showcase-content-*.zip --relax=false --accept=true
fi
gosu $NUXEO_USER $NUXEO_HOME/bin/nuxeoctl mp-list|grep platform-explorer
if [ $? -eq 1 ]; then
    gosu $NUXEO_USER $NUXEO_HOME/bin/nuxeoctl mp-install /docker-entrypoint-initnuxeo.d/platform-explorer-*.zip --relax=false --accept=true
fi
gosu $NUXEO_USER $NUXEO_HOME/bin/nuxeoctl mp-list|grep virtualnavigation
if [ $? -eq 1 ]; then
    gosu $NUXEO_USER $NUXEO_HOME/bin/nuxeoctl mp-install /docker-entrypoint-initnuxeo.d/nuxeo-virtualnavigation-*.zip --relax=false --accept=true
fi
gosu $NUXEO_USER $NUXEO_HOME/bin/nuxeoctl mp-list|grep template-rendering-samples
if [ $? -eq 1 ]; then
    gosu $NUXEO_USER $NUXEO_HOME/bin/nuxeoctl mp-install /docker-entrypoint-initnuxeo.d/nuxeo-template-rendering-samples-*.zip --relax=false --accept=true
fi
